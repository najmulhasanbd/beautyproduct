(function ($) {
	...
	function updateCountdown() {
		...
		if (countdownItems.length > 0) {
			...
			countdownItems[0].querySelector(".countdown-value").textContent = String(
				days,
			).padStart(2, "0");
			countdownItems[1].querySelector(".countdown-value").textContent = String(
				hours,
			).padStart(2, "0");
			countdownItems[2].querySelector(".countdown-value").textContent = String(
				minutes,
			).padStart(2, "0");
			countdownItems[3].querySelector(".countdown-value").textContent = String(
				seconds,
			).padStart(2, "0");
		}
	}
	...
	$(document).ready(function () {
		$(".mobileCategory li a.has-child").on("click", function (e) {
			// মোবাইল ভিউতে ড্রপডাউন ওপেন করবে কিন্তু পেজ রিলোড হবে না
			e.preventDefault();
			e.stopPropagation();
			...
			if ($parentLi.hasClass("active")) {
				...
				// অন্য সব ওপেন মেনু বন্ধ করা
				$parentLi.siblings().removeClass("active").children("ul").slideUp(300);

				// বর্তমান মেনু ওপেন করা
				$parentLi.addClass("active");
				$targetUl.stop(true, true).slideDown(300);
			}
		});
	});
	...
	document.addEventListener("DOMContentLoaded", function () {
		...
		hotspots.forEach((hotspot) => {
			hotspot.addEventListener("mouseenter", function () {
				...
				if (targetProduct) {
					document
						.querySelectorAll(".shopSingleProduct.active-highlight")
						.forEach((el) => {
		...
		productItems.forEach((item) => {
			item.addEventListener("mouseenter", function () {
				...
				const matchingHotspot = document.querySelector(
					`.look-hotspot[data-target-product="${productId}"]`,
				);
